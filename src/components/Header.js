...
const Header = () => {
  const [loginBtn, setBtn] = useState("Login");
  const cartItems = useSelector((store) => store.cart.items);
  console.log(cartItems);

  return (
    <div className="flex p-2 justify-between bg-yellow-100 shadow-lg sticky top-0">
      <Title />
      <div>
        <ul className="flex py-2 ">
          ...
          <button
            className="border px-1 rounded-md w-15 bg-gray-400"
            onClick={() => {
              loginBtn === "Login" ? setBtn("Logout") : setBtn("Login"); //if login-> logout, otherwise keep login
